...
describe("Material", function() {
    ...
    it.skip('should not be able to use more material than available', function(){
        ...
        assert.equal(paint.useMaterial(12), 'You don\'t have enough acryic paint! Try using 7 bottles or less.');
        assert.equal(canvas.useMaterial(4), 'You don\'t have enough canvas! Try using 2 sheets or less.');
    })

    it.skip('should be able to calculate total costs of materials', function() {
        var paint = new Material('acrylic paint', 5 , 7, 'bottles');
        var canvas = new Material('canvas', 15, 2, 'sheets');

        assert.equal(paint.calculateMaterialCosts(), 35);
        assert.equal(canvas.calculateMaterialCosts(), 30);
    })
});