...
var GolfCourse = require('../src/golf-course');
var Golfer = require('../src/golfer');
...
describe('Golf Course', function () {
    ...
    it.skip('should have some unique features', function() {
        ...
        assert.deepEqual(golfCourse1.features, ['great views', 'wilflife']);
        assert.deepEqual(golfCourse2.features, ['water', 'Arnies Place']);
    });

    // PASS GOLFER TESTS BELOW BEFORE FINISHING REAINING GOLF COURSE TESTS
    ...
    it.skip('should not accept golfers if there is no room available', function() {
        ...
        assert.equal(golfCourse.currentlyPlaying, ['Kevin', 'Ted', 'Pat']);
...
describe('Golfer', function() {
    ...
    it.skip('should start out minimally frustrated', function() {
        ...
        assert.equal(golfer.frustation, 0);
    });
    ...
    it.skip('should become rustrated based on the course difficulty (HARD or MODERATE - golf is never easy', function() {
        ...
        assert.equal(golfer1.frustation, 500);

        golfer2.playRound(golfCourse2);
        assert.equal(golfer2.frustation, 100);
    });
