...
describe('Hobbit', function() {
    ...
    it.skip('should be considered an child at 32', function() {
        var hobbit = new Hobbit({ name: 'Taylor' });

        timeTravel(32, hobbit);

        assert.equal(hobbit.age, 32);
        assert.equal(hobit.adult, false);
    });

    it.skip('should be considered a child at the age of 33', function() {
        var hobbit = new Hobbit({ name: 'Taylor' });
    ...
    it.skip('should be able to get the ring if its name is Frodo', function(){
        ...
        assert.equal(hobbit1.hasRing, false);
    });
});